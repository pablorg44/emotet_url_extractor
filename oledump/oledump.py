...
def DecodeDumpedData(data):
    if sys.version_info[0] > 2:
        base64 = re.sub("^b\'\\x.*\\x[0-9]{1,2}((?!\\x))", '', str(C2BIP3(data)))
        print(base64)
    else:
        while data != '':
            sys.stdout.write(data[0:10000])
...
def OLESub(ole, prefix, rules, options):
    ...
    if options.select == '':
        counter = 1
        vbaConcatenate = ''
        # Añadimos macros para devolverlas
        macros=[]
        for orphan, fname, entry_type, entry_clsid, stream in OLEGetStreams(ole, options.storages):
            indicator = ' '
            macroPresent = False
            ...
            if not options.quiet:
                ...
                #print(line)
                # Añadimos cada línea al array de macros para, posteriormente, buscarlas
                macros.append(line)
        ...
        if options.select.endswith('c') or options.select.endswith('s'):
            ...
            part = ''
        for orphan, fname, entry_type, entry_clsid, stream in OLEGetStreams(ole, options.storages):
            if (selection == 'a' or ('%s%d' % (prefix, counter)) == selection.replace(' ','')) or (prefix == 'A' and str(counter) == selection):
                # Modificado para coger la macro dumpeada
                macro_dump = StdoutWriteChunked(HeadTail(DumpFunction(DecompressFunction(DecodeFunction(decoders, options, CutData(SelectPart(stream, part, dModuleinfo.get(''.join([c + '\x00' for c in fname[-1]]), None)), options.cut)[0]))), options.headtail))
                return macro_dump
                selectionCounter += 1
                if selection != 'a':
                    break
...
def OLEDump(filename, options):
    ...
    if options.find != '':
        ...
        if magic[0:4] == OLEFILE_MAGIC:
            ...
            macros.append(OLESub(ole, '', rules, options))
            #PrintWarningSelection(options.select, selectionCounter)
            ole.close()
            return macros
        elif magic[0:2] == b'PK':
            ...
            for info in oZipfile.infolist():
                ...
                if content[0:4] == OLEFILE_MAGIC:
                    ...
                    macros.append(OLESub(ole, letter, rules, options))
                    #selectionCounterTotal += selectionCounter # COmentamos esto que no es necesario
                    oleFileFound = True
                    ole.close()
                oZipContent.close()
            if not oleFileFound:
            ...
            oZipfile.close()
            # Modificación para retornar las macros en forma de lista
            return macros
        else:
            data = oStringIO.read()
            oStringIO.seek(0)
